    ...
    async execute(message, args, client, config, colors, createYakuzaEmbed, emojis) {
        ...
        if (!target) {
            const errorEmbed = createYakuzaEmbed(
                'Usuário Inválido',
                'Por favor, mencione um usuário para abraçar.',
                colors.error
            );
            return await message.reply({ embeds: [errorEmbed] });
        }
        
        await performRoleplay('hug', message.author, target, message, null, config, colors, createYakuzaEmbed, emojis);
    },
    
    async executeSlash(interaction, client, config, colors, createYakuzaEmbed, emojis) {
        const target = interaction.options.getUser('usuário');
        await performRoleplay('hug', interaction.user, target, null, interaction, config, colors, createYakuzaEmbed, emojis);
    }
};
    ...
    try {
        if (author.id === target.id) {
            const selfEmbed = createYakuzaEmbed(
                'Ação Inválida',
                'Você não pode se abraçar! Merdinha solitário(a).',
                colors.error
            );
        ...
        const messages = {
            hug: [
                `**${author.username}** deu um abraço apertado em **${target.username}**`,
                `**${author.username}** abraçou **${target.username}** carinhosamente`,
                `**${author.username}** abraçou **${target.username}**`
            ]
        };
